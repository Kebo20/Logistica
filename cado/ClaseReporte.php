<?php
   ...
   class Reportes{
	...
	function ListarSerie(){
	  $ocado=new cado();
	  $sql="select tipo_doc,concat(tipo_doc,' - ',serie) ser from serie where tipo_doc in('FA','BV','NC')"; 
	  $ejecutar=$ocado->ejecutar($sql);
	  return $ejecutar;
	}
	function ListarSerieInnova(){
	  $ocado=new cado();
	  $sql="select tipo_doc,concat(tipo_doc,' - ',serie) ser from serie where tipo_doc in('TB','TF')"; 
	  $ejecutar=$ocado->ejecutar($sql);
	  return $ejecutar;
	}
	function ListaReporDoc($ini,$fin,$pac,$serie,$tipo){
	  ...
	  $sql="select d.id,serie,correlativo,case when d.tipo_documento='03' then  concat(ape_pat,' ',ape_mat,' ',preNombres) else nomcli end pac,importe_total,xml_generado,resp_cdr,
	        ...
	        where date(fecha_emision) >='$ini' and date(fecha_emision)<='$fin' and d.tipo_documento in('01','03','07') $where 
	...
	function ListaReporDocInnova($ini,$fin,$pac,$serie,$tipo){
	  ...
	  $sql="select d.id,serie,correlativo,case when d.tipo_documento='03' then  concat(ape_pat,' ',ape_mat,' ',preNombres) else nomcli end pac,importe_total,xml_generado,resp_cdr,
	        ...
	        from doc_electronicos d left join paciente p on d.id_cliente=p.id 
	        where date(fecha_emision) >='$ini' and date(fecha_emision)<='$fin' and d.tipo_documento in('TB','TF') $where 
			order by serie,correlativo desc "; 
	  $ejecutar=$ocado->ejecutar($sql);
	  return $ejecutar;
	}
	...
	function LisEmpresaPagos($est){
		  ...
		  $sql="select c.id,c.empresa from doc_electronicos doc 
				  ...
				  where cu.estado=0 and pagado=$est
  ...
  '$fin','$emp')";	
	 ...
	 function pagarTickets($idticket,$nro_ticket,$monto,$user,$retencion,$pago,$dni,$des,$per,$tipo,$dref){
		try{
		  $ocado=new cado();
		  $cn=$ocado->conectar();
		  $cn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
...
values('$cod','$idticket','EG','T-$nro_ticket','E',now(),now(),0,'$pago','PAGO MEDICO','P','$monto','$retencion','$dni','$des','$per','$tipo','$dref');";
	...
	total_isc,total_otrtri,total_otrcar,total_glosa,valor_venta,importe_total,created_at,id_cliente,grupal,titulo_gratuito)  	  values('$cod_operacion',now(),now(),'$cod_dom_fiscal','$tipo_documento','$serie','$correlativo','$tipodoc_cli','$doc_cliente','$nomcli',
	'$dircliente','$moneda','$op_gravadas','$op_gratuitas','$op_inafecta','$op_exoneradas','$total_descuento','$total_igv','$total_isc',
'$total_otrtri','$total_otrcar','$total_glosa','$valor_venta','$importe_total',now(),0,1,$titulo);";
		  ...
		  // 16- tipo_isc (null por defecto) , 17- valisc(null por defecto) , 18- created_at(fecha hora del sistema), 19- updated_at(null por defecto)
			 ...
			 // Falta para detallado
			  ...
			  $sql_detalle="INSERT INTO `doc_electronico_items` (`doc_electronico_id`, `nro_orden`, `tipo_item`, `cod_afecta_igv`, `unidad_medida`, `cantidad`, `descripcion_item`,
		                    ...
		                    VALUES ('$doc_electronico_id', 1, 2, '10', 'ZZ', 1, 'POR CONCEPTOS DE ANALISIS CLINICOS','','$valuni','$valigv', '$preuni', '$valuni', now());";
		  ...
		  $sql_totales="update doc_electronicos set total_igv=$valigv,op_gravadas=$valuni,valor_venta=$valuni
